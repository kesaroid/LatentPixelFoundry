...
Use this to test the pipeline on the EC2 instance (or inside the worker container).
  ...
  Inside the worker container (torch/diffusers are installed there; /models is mounted):
    docker exec -it lpf-worker python3 /app/scripts/run_prompt.py --prompt "A cat walking in the rain" --duration 2 --resolution 480p --output /tmp/test.mp4
...
def main() -> None:
    ...
    # Smoke-test defaults: validate model/pipeline loading quickly.
    p.add_argument("--duration", "-d", type=float, default=2.0, help="Duration in seconds (default: 2.0)")
    p.add_argument(
        "--resolution",
        "-r",
        default="480p",
        choices=["480p", "720p", "1080p"],
        help="Resolution (default: 480p)",
    )
    p.add_argument(
        "--stage1-steps",
        type=int,
        default=8,
        help="Stage-1 diffusion steps (default: 8). Lower is faster but lower quality.",
    )
    ...
    print(
        f"[run_prompt] prompt={args.prompt!r} duration={args.duration}s resolution={args.resolution} stage1_steps={args.stage1_steps} output={args.output}"
    )
    out = run_ltx2(
        args.prompt,
        ...
        num_inference_steps_stage1=args.stage1_steps,
        seed=args.seed,
    )
